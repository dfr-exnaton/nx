...
export function sourcesRenderer(
  ...
  return ({ rows, stylesheet }: any) => {
    return rows.map((node: any, idx: number) => {
      ...
      return (
        <span className="group/line flex min-w-0" key={`code-group${idx}`}>
          <span>{element}</span>
          {sourceElement && (
            <span className="inline flex min-w-0 pl-2 opacity-0 transition-opacity duration-150 ease-in-out group-hover/line:opacity-100">
              {sourceElement}
            </span>
          )}
        </span>
